...
- Mobile status bar (2026-07-26): keep the iOS/webview status area immersive with the active page theme. Light mode uses a neutral paper `#f7f6f2`; dark mode uses deep indigo `#0d1430`. An inline head script restores the saved theme before first paint; React immediately updates `theme-color`, `color-scheme`, and the root/body background on theme switches. Note: native iOS in-app browser chrome remains host-controlled and may not change dynamically.
